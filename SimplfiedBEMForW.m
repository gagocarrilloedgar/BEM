...
switch(option)
    case 1
        while(F1>0)
            [F1] =  WdFz(omega,R_propeller,rho,Chord_real,Cl_modified,lambdai_BEM,Cd_modified,N,Weight);
            omega = omega*1.5;
        end
    case 2 % Numerical integration in order to find the right 
        while(F1>0)
            [F1] =  WdFzNumeric(omega,R_propeller,rho,Chord_real,Cl_modified,lambdai_BEM,Cd_modified,N,Weight,r);
            omega = omega + 100;
        end
    otherwise
end
